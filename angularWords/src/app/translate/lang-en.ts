export const LANG_EN_NAME = 'en';

export const LANG_EN_TRANS = {
    "es": "Spanish",
    "en": "English",
    
    "github": "GitHub repository",

    "brand": "oriWords",
  
    "welcome": "Welcome",
    
    "login": "Login",
    "login.succes": "Logged in successfully",
    "login.username.placeholder": "Email / username",
    "login.username.required": "Email / username is required",
    "login.verified": "Your account have been verified successfully",
    "login.noverified": "Your account is not confirmed yet, go to your email and click the link",
    "login.verified.already": "Your account is already confirmed",
    
    "signup": "Sign-up",
    "signup.success": "Registered successfully, go to your email and confirm your account",
    
    "email.incorrect": "Incorrect username/email",
    "email.placeholder": "Email",
    "email.exists": "Email already in use",
    "email.format": "Email format is not correct",
    "email.required": "Email is required",

    "username.placeholder": "Username",
    "username.exists": "Username already in use",
    "username.minlength": "Username must be at least 6 characters long.",
    "username.maxlength": "Username cannot exceed 15 characters.",
    "username.required": "Username is required",
    
    "password.placeholder": "Password",
    "password.confirm.placeholder": "Confirm password",
    "password.invalid": "Invalid password",
    "password.nomatch": "Password has to match",
    "password.minlength": "Password must be at least 6 characters long.",
    "password.maxlength": "Password cannot exceed 15 characters.",
    "password.required": "Password is required",
    "password.forgot": "Forgot your password?",
    "password.recover": "Recover password",
    "password.recover.success": "Mail with recovery instructions sent",
    "password.new": "New password",
    "password.reset": "Reset password",
    "password.reset.success": "Password have been change",
    "password.reset.invalid": "Invalid token",
    "password.reset.expired": "Token expired",
    
    "actions": "Actions",
    "actions.custom": "Custom actions",
    "row.add": "Add row",
    "row.cancel": "Cancel",
    "row.create": "Create",
    "row.created": "Row created successfully",
    "row.delete": "Delete row",
    "row.deleted": "Row deleted successfully",
    "row.delete.confirm": "Confirm and delete",
    "row.edit": "Edit row",
    "row.edited": "Row edited successfully",
    "row.save": "Save changes",

    "filter": "filter...",

    "users": "Users",
    "user.role": "Role",
    "user.email.confirmed": "Email confirmed",
    "user.created": "User created successfully",
    "user.deleted": "User deleted successfully",
    "user.edited": "User edited successfully",

    "friends": "Friends",
    "friends.add": "Add friend",
    "friends.pending": "Pendings",
    "friends.connected": "Connected",
    "friends.disconnected": "Disconnected",

    "ligas": "Competitions",
    "ligas.name.placeholder": "Name",
    "ligas.idApi.placeholder": "API id",
    "ligas.select": "Select a competition",

    "jornadas": "Matchdays",
    "jornadas.liga.placeholder": "Competition",
    "jornadas.inicio.placeholder": "Start",
    "jornadas.fin.placeholder": "End",
    "jornadas.matchday.placeholder": "Matchday",
    
    "search": "Search",

    "value": "Value",

    "error.unknown": "Unknown error"
};